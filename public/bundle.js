...
var ActionTypes = {
  ...
  }, _ref2[index$1] = observable, _ref2;
}
...
var store = createStore(combineReducers({ count: count }), initStore, window.__REDUX_DEVTOOLS_EXTENSION__ && window.__REDUX_DEVTOOLS_EXTENSION__());
...
var template = function () {
	return {
		data: function data() {
			return { count: 0, store: store };
		},
		oncreate: function oncreate() {
			var set$$1 = this.set;
			var boundSet = set$$1.bind(this);
			store.subscribe(function () {
				var _store$getState = store.getState(),
				    count = _store$getState.count;

				boundSet({ count: count });
			});
		},
		...
		methods: {
			...
			decrement: function decrement(dispatch) {
...
function create_main_fragment$1(state, component) {
	var div, button, text, text_1, span, text_2_value, text_2, text_3, button_1, text_4;

	function click_handler(event) {
		var state = component.get();
		component.decrement(state.store.dispatch);
	}

	function click_handler_1(event) {
		var state = component.get();
		component.increment(state.store.dispatch);
	}
	...
	return {
		create: function create() {
			...
			text_2 = createText(text_2_value = state.count);
			text_3 = createText("\n  ");
...
assign(Counter.prototype, template.methods, proto);
...
function create_main_fragment(state, component) {
	...
	return {
		create: function create() {
			counter._fragment.create();
		},

		mount: function mount(target, anchor) {
			counter._fragment.mount(target, anchor);
		},

		unmount: function unmount() {
			counter._fragment.unmount();
		},
